...
log_file = './logs/localdownloadlog_%s.txt' % time.strftime("%Y%m%d-%H%M%S")
def write_to_log(m):
    print(m)
...
def parallelize_download_from_local(chunk):
    ...
    with pysftp.Connection('malloy.princeton.edu', username='qle', password=secret_pw) as sftp:
        for i in range(len(chunk)):
            with sftp.cd('locca_revolve/'):
                ...
                mkdir_p(out_folder)            
                # mkdir_p('/'.join(['scans', chunk[i][1]]))
                # print(chunk[i][1])
                sftp.get(chunk[i][1], chunk[i][1])
    return 0          
...
def main():
    ...
    time0 = time.time()
    results = list(tqdm(pool.imap(parallelize_download_from_local, my_chunks)))
    time1 = time.time()
